"""
Train a RandomForest model to predict next-step sensor values.

This script mirrors the ETL pipeline flow:
...
- Trains a RandomForest-based multi-output regressor per transition or across
  all transitions.

Usage:
    python models/random_forest_next_values.py --data-root data/raw --max-files 400
    python models/random_forest_next_values.py --transition mashing->boiling
"""
...
def resolve_data_root(data_root: Path) -> Path:
    """Resolve common project layouts for raw parquet data."""
    candidates = [data_root, Path("ETL_pipeline/data/raw")]
    for candidate in candidates:
        if any(candidate.glob("date=*/beer-*.parquet")):
            return candidate
    searched = ", ".join(str(c) for c in candidates)
    raise FileNotFoundError(f"No parquet files found. Searched: {searched}")
...
def load_step_stats(data_root: Path, max_files: int | None) -> pd.DataFrame:
    ...
    files = sorted(resolved_root.glob("date=*/beer-*.parquet"))
    if max_files:
...
def build_transition_rows(step_stats: pd.DataFrame) -> pd.DataFrame:
    """Create rows with features from completed steps and targets for the next step."""
...
def train_transition_model(
    ...
    """Train and evaluate a RandomForest model for a specific step transition."""
    ...
    sample_pred = model.predict(sample_features.to_frame().T)[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train RandomForest models for next-step predictions.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional transition to train (e.g., mashing->boiling).",
    )
    args = parser.parse_args()

    resolved_root = resolve_data_root(args.data_root)
    print(f"Loading data from {resolved_root} (max_files={args.max_files}) ...")
    step_stats = load_step_stats(resolved_root, args.max_files)
    ...
    print(
        f"Built {len(transitions_df)} transition examples across {transitions_df.transition.nunique()} transitions."
    )

    transitions = (
        [args.transition] if args.transition else sorted(transitions_df.transition.unique())
    )
